  ...
  const groups = useMemo(() => {
    ...
    return targets
      .map((y) => {
        ...
        const sameYear = entries.filter(
          (e) => e.entryDate.startsWith(`${year}-`) && e.entryDate.slice(5) === md,
        )
        return {
          years: y,
          label: `${y} year${y === 1 ? '' : 's'} ago — ${targetIso}`,
          entries: exact.length ? exact : sameYear,
        }
      })
  ...
  if (groups.length === 0) {
    return (
      <div className="mem-empty">
        <p>
          No memories yet for today’s date. Keep writing — entries will surface here in 1, 2, 3, and
          5 years.
        </p>
