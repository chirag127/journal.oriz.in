 ...
 *
 * Lazy proxy pattern (vendored from oriz-home so we no longer depend on
 * @chirag127/oriz-ui's `initFirebase` helper). Server-side prerender of pages
 * that import a React island doesn't crash when env vars are missing on the
 * build runner — only the BROWSER needs the auth instance, and the React
 * component runs only there.
 */
