...
export async function upsertProfile(uid: string, profile: Partial<UserProfile>) {
  await setDoc(doc(db, userPath(uid)), { ...profile, updatedAt: Date.now() }, { merge: true })
}
...
export async function listEntries(
  ...
  else if (opts.journalType)
    q = query(
      ref,
      where('journalType', '==', opts.journalType),
      orderBy('entryDate', 'desc'),
      limit(opts.limit ?? 200),
    )
...
export async function saveTemplate(uid: string, t: Template) {
  const { id, ...data } = t
  await setDoc(
    doc(db, `${userPath(uid)}/templates/${id}`),
...
export async function getStreak(uid: string): Promise<StreakCounter> {
  ...
  if (!snap.exists())
    return { current: 0, longest: 0, lastEntryDate: null, totalEntries: 0, totalWords: 0 }
